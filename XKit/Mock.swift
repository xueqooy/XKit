...
public struct MockOption<T : Mockable> {
    ...
    /// Mock for Optional
    public static func success<V: Mockable>(delay: TimeInterval = 2, allowsNil: Bool = true) -> MockOption where T == Optional<V> {
        .init(delay: delay) {
    ...
    /// Mock for Array
    public static func success<V: Mockable>(delay: TimeInterval = 2, elementCountRange: ClosedRange<Int>) -> MockOption where T == Array<V> {
        .init(delay: delay) {
    ...
    /// Mock for Array
    public static func success<V: Mockable>(delay: TimeInterval = 2, elementCount: Int) -> MockOption where T == Array<V> {
        .init(delay: delay) {
    ...
    /// Mock for Dictionary
    public static func success<K: Mockable, V: Mockable>(delay: TimeInterval = 2, keys: [K], allowsNilValue: Bool = false) -> MockOption where T == Dictionary<K, V> {
        .init(delay: delay) {
...
extension Optional: Mockable where Wrapped: Mockable {
    public static func mock() -> Optional<Wrapped> {
...
extension Array: Mockable where Element: Mockable {
    ...
    public static func mock(elementCount: Int) -> Array<Element> {
        (0..<elementCount).map { _ in
            Element.mock()
        }
    }
    
    public static func mock(elementCountIn range: ClosedRange<Int>) -> Array<Element> {
        let count = Int.random(in: range)
...
extension Dictionary: Mockable where Key: Mockable, Value: Mockable {
    public static func mock() -> Dictionary<Key, Value> {
        var dict = Self.init()
    ...
    public static func mock(keys: [Key], allowsNilValue: Bool = false) -> Dictionary<Key, Value> {
        var dict = Self.init()
